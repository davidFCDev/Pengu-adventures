...
export class SurfaceDetector {
  ...
  static findValidSurfaces(
    ...
    console.log(
      `🔍 Buscando superficies en mapa de ${layer.width}x${layer.height} tiles...`
    );
    ...
    console.log(`✅ Total superficies encontradas: ${surfaces.length}`);
    return surfaces;
  }
  ...
  private static createSurface(
    ...
    if (surfaceWidth < minTilesWidth) {
      console.log(
        `❌ Superficie rechazada: width=${surfaceWidth} tiles < ${minTilesWidth} tiles mínimo`
      );
      return null;
    }
    ...
    for (const area of excludeAreas) {
      ...
      if (distance < area.radius) {
        console.log(
          `❌ Superficie rechazada: dentro de área excluida (${distance.toFixed(
            0
          )}px < ${area.radius}px)`
        );
        return null;
    ...
    console.log(
      `✅ Superficie válida: width=${surfaceWidth} tiles (${
