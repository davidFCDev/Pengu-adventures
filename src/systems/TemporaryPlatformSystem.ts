...
export class TemporaryPlatformSystem {
  ...
  private checkPlayerOnPlatforms(): void {
    ...
    this.platformStates.forEach((state, key) => {
      ...
      // Verificar si el player está SOBRE la plataforma
      ...
      const isVerticallyAligned =
        playerBottom >= platformTop - 5 && playerBottom <= platformBottom + 5;

      const isHorizontallyAligned =
        playerRight > platformLeft && playerLeft < platformRight;

      const isLanding = playerBody.velocity.y >= -10 && playerBody.blocked.down;

      const isOnPlatform =
        isVerticallyAligned && isHorizontallyAligned && isLanding;
