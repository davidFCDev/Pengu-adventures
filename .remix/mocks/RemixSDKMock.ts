...
function getGameStateKey() {
  ...
  const cleanPath =
    path.replace(/[^a-zA-Z0-9-]/g, "_").replace(/^_+|_+$/g, "") || "default";
...
class RemixSDKMock {
  ...
  public instanceId: string; // Public para acceso externo en broadcast
  ...
  singlePlayer = {
    actions: {
      ready: () => {
        ...
        if (this.isMultiplayer) {
          ...
          setTimeout(() => {
            ...
            const meId =
              urlParams.get("player") || urlParams.get("instance") || "1";
      ...
      gameOver: (
        data: { score?: number; finalScore?: number; highScore?: number } = {}
      ) => {
      ...
      saveGameState: ({
        ...
        // Update global state and share it (reuse multiplayer logic for consistency)
        multiplayerState.gameState = { id: gameStateId, data: gameState };
        updateSharedState(multiplayerState);

        // Emit game_state_updated event so the game can react to the save
        this.emit("game_state_updated", { id: gameStateId, gameState });

        // Notify parent window for dev UI
        this.postToParent("game_state_updated", {
          id: gameStateId,
          data: gameState,
        });

        console.log("💾 Single Player: Game state saved", gameState);
      },
        ...
        setTimeout(() => {
          ...
          setTimeout(() => {
            ...
            if (sharedState.gameState) {
              console.log(
                "Sending existing game state on ready:",
                sharedState.gameState
              );
              this.emit("game_state_updated", sharedState.gameState);
            } else {
              console.log(
                "No existing game state, sending null to start new game"
              );
  ...
  if (isDev) {
    ...
    window.addEventListener("message", (event) => {
      if (event.data && event.data.type === "remix_dev_command") {
        ...
        event.data &&
        event.data.type === "multiplayer_game_state_broadcast"
      ) {
